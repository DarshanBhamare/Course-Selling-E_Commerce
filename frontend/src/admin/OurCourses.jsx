...
function OurCourses() {
    ...
    const fetchCourses = async () => {
      try {
        ...
        // Force slick to recalculate after data load
        setTimeout(() => {
...
//delete course
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:4002/api/v1/course/delete/${id}`, {
  ...
  return (
    <div className="bg-gray-100 p-8 space-y-4">
      ...
      <Link
        className="bg-pink-400 py-2 px-4 rounded-lg text-white hover:bg-orange-950 duration-300"
        to={"/admin/dashboard"}
      >
        Go to dashboard
      </Link>
          ...
          <div key={course._id} className="bg-white shadow-md rounded-lg p-4">
            ...
            <div className="flex justify-between">
              <Link
                ...
                className="bg-pink-500 text-white py-2 px-4 mt-4 rounded hover:bg-blue-600"
              >
                Update
              </Link>
              <button
